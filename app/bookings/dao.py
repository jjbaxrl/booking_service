    ...
    async def add(cls, user_id: int, room_id: int, date_from: date, date_to: date):
        ...
        try:
            async with async_session_maker() as session:
                rooms_check = select(Rooms.id)
                res = await session.execute(rooms_check)
                rooms = res.scalars().all()  # Получаем все ID комнат
                ...
                query = (
                    select(Rooms.quantity - booked_rooms.c.booked_count)
                    .select_from(Rooms)
                    .join(
            ...
            logger.error("Room can't be booked Exc", extra=extra, exc_info=True)
